...
这是根据王晋东的DeepDA Toolkit扩展而来的，适用于1D振动信号的Diagnosis Domain Adaptation Toolbox。我在原先库的基础上添加了我所使用的网络Backbone。
并且添加了使用Wasserstein距离的WD方法、融合全局与局部Wasserstein 距离的LWD方法。
此外，ZXJB_train.py文件实现的是转向架缩比试验台数据 多工况的遍历训练，可直接输出得到训练过程和结果，存储在/Record 中的csv文件中，方便实现方法的对比。
其他数据集的遍历训练仍在整理中，有待进一步更新。
